    ...
    public function up(): void
    {
        Schema::table('cart_items', function (Blueprint $table) {
            if (!Schema::hasColumn('cart_items', 'product_variation_id')) {
                $table->foreignId('product_variation_id')->nullable()->constrained()->nullOnDelete()->after('product_id');
            }
            if (!Schema::hasColumn('cart_items', 'selected_attributes')) {
                $table->json('selected_attributes')->nullable()->after('product_variation_id');
            }
        });
    }
