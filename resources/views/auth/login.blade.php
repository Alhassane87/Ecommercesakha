<x-guest-layout>
    <div class="fixed inset-0 overflow-y-auto bg-slate-100">
        <div class="pointer-events-none absolute inset-0 overflow-hidden">
            <div class="absolute -left-24 -top-20 h-80 w-80 rounded-full bg-cyan-300/35 blur-3xl"></div>
            <div class="absolute right-0 top-1/3 h-72 w-72 rounded-full bg-emerald-300/30 blur-3xl"></div>
            <div class="absolute bottom-0 left-1/3 h-72 w-72 rounded-full bg-blue-400/25 blur-3xl"></div>
        </div>
        ...
        <div class="relative mx-auto flex min-h-full w-full items-center justify-center px-4 py-6 sm:px-6 lg:px-8">
            <div class="grid w-full max-w-6xl overflow-hidden rounded-3xl border border-white/70 bg-white/90 shadow-[0_28px_90px_-35px_rgba(15,23,42,0.45)] backdrop-blur-xl lg:grid-cols-[1.05fr_1fr]">
                <aside class="relative hidden lg:flex flex-col justify-between overflow-hidden bg-slate-900 p-10 text-white">
                    <div class="absolute inset-0 bg-gradient-to-br from-cyan-500 via-sky-500 to-emerald-500 opacity-90"></div>
                    ...
                    <div class="relative">
                        ...
                        <h2 class="mt-6 text-4xl font-black leading-tight">Bienvenue sur Sakha</h2>
                        <p class="mt-4 text-sm leading-6 text-white/90">
                            Connectez-vous pour suivre vos commandes, gerer votre panier et profiter de vos offres personnalisees.
                        </p>
                    </div>
                    ...
                    <div class="relative space-y-4">
                        <div class="rounded-2xl border border-white/30 bg-white/15 p-4 backdrop-blur-sm">
                            ...
                            <p class="mt-1 text-xs text-white/85">Reception en 24-48h sur vos achats eligibles.</p>
                        </div>
                        <div class="rounded-2xl border border-white/30 bg-white/15 p-4 backdrop-blur-sm">
                            <p class="text-sm font-semibold">Paiement securise</p>
                            <p class="mt-1 text-xs text-white/85">Transactions protegees avec verification renforcee.</p>
                        </div>
                        <div class="rounded-2xl border border-white/30 bg-white/15 p-4 backdrop-blur-sm">
                            <p class="text-sm font-semibold">Support disponible</p>
                            <p class="mt-1 text-xs text-white/85">Notre equipe vous accompagne 7j/7.</p>
                        </div>
                    </div>
                </aside>
                ...
                <section class="max-h-[95vh] overflow-y-auto p-6 sm:p-9 lg:p-10">
                    <div class="mx-auto w-full max-w-md">
                        ...
                        <div class="mb-8">
                            ...
                            <h1 class="mt-4 text-3xl font-black text-slate-900 sm:text-4xl">Accedez a votre compte</h1>
                            <p class="mt-2 text-sm text-slate-600">
                                Heureux de vous revoir. Entrez vos informations pour continuer.
                            </p>
                        </div>
                        ...
                        @if ($errors->any())
                            <div class="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                                Verifiez les informations saisies puis reessayez.
                            </div>
                        @endif
                        ...
                        <form method="POST" action="{{ route('login') }}" class="space-y-5">
                            ...
                            <div>
                                ...
                                <x-text-input
                                    id="email"
                                    class="mt-2 block w-full rounded-xl border-slate-200 bg-white px-4 py-3 text-slate-900 shadow-sm transition focus:border-cyan-500 focus:ring-cyan-500"
                                    type="email"
                                    name="email"
                                    :value="old('email')"
                                    required
                            ...
                            <div>
                                <div class="flex items-center justify-between">
                                    ...
                                    @if (Route::has('password.request'))
                                        <a class="text-xs font-semibold text-slate-500 transition hover:text-cyan-700" href="{{ route('password.request') }}">
                                            Mot de passe oublie ?
                                        </a>
                                    @endif
                                </div>
                                ...
                                <div class="relative mt-2">
                                    <x-text-input
                                        id="password"
                                        class="block w-full rounded-xl border-slate-200 bg-white px-4 py-3 pr-12 text-slate-900 shadow-sm transition focus:border-cyan-500 focus:ring-cyan-500"
                                        type="password"
                                        name="password"
                                        required
                            ...
                            <label class="inline-flex items-center gap-2 text-sm text-slate-600">
                                <input id="remember" type="checkbox" name="remember" class="rounded border-slate-300 text-cyan-600 focus:ring-cyan-500">
                                <span>Se souvenir de moi</span>
                            </label>

                            <x-primary-button class="w-full justify-center rounded-xl border-0 bg-gradient-to-r from-sky-500 via-cyan-500 to-emerald-500 px-5 py-3 text-sm font-bold normal-case tracking-normal text-white shadow-lg shadow-cyan-500/30 transition hover:scale-[1.01] hover:from-sky-600 hover:to-emerald-600 focus:ring-cyan-500">
                                Se connecter
                            </x-primary-button>
                        </form>
                        ...
                        <p class="mt-6 text-center text-sm text-slate-600">
                            Nouveau sur Sakha ?
                            <a href="{{ route('register') }}" class="font-semibold text-cyan-700 transition hover:text-cyan-800">
                                Creer un compte
                            </a>
                        </p>
                    </div>
